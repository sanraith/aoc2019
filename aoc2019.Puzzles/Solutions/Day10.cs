...
namespace aoc2019.Puzzles.Solutions
{
    ...
    public sealed class Day10 : SolutionBase
    {
        ...
        public override async Task<string> Part2Async(string input)
        {
            ...
            var allDestroyedAsteroids = new List<Point>();
            ...
            throw new InvalidOperationException("Not enough asteroids!");
        }
        ...
        private async Task<Dictionary<Point, int>> GetDetectedAsteroids(List<Point> asteroids)
        {
            ...
            foreach (var (proposedAsteroid, index) in asteroids.Select((x, i) => (x, i)))
            {
                ...
                var detectedAngles = new List<double>();
                foreach (var asteroid in asteroids)
                {
                    if (asteroid == proposedAsteroid) { continue; }
                    var vector = proposedAsteroid - asteroid;
                    var angle = vector.Angle.Value;
                    if (detectedAngles.All(d => Math.Abs(d - angle) >= double.Epsilon))
                    {
        ...
        internal struct Point : IEquatable<Point>
        {
            ...
            public Point(int x, int y)
            {
                ...
                else
                {
                    ...
                    //Angle = (-1 * Angle + Math.PI * 2.5) % (Math.PI * 2);
                    Angle = (Math.PI / 2 - Angle + 2 * Math.PI) % (2 * Math.PI);
                    //new[] { Math.Atan2(1, 0), Math.Atan2(0, 1), Math.Atan2(-1, 0), Math.Atan2(0, -1), Math.Atan2(1, -1) }.Select(x => Math.PI / 2 - x).ToArray()
                }
            }
